...
import '../../controller/costroomcontroller.dart';
...
class CostRoomDialog extends StatefulWidget {
  final String? idRom;
  final DateTime? startDate;
  final DateTime? endDate;
  const CostRoomDialog({Key? key, this.idRom, this.startDate, this.endDate})
      : super(key: key);

  @override
  State<CostRoomDialog> createState() => _CostRoomDialogState();
}

class _CostRoomDialogState extends State<CostRoomDialog> {
  late CostRoomController controller;
  double totalAmout = 0;
  ...
  void initState() {
    super.initState();
    controller =
        CostRoomController(widget.idRom!, widget.startDate, widget.endDate);
  }
    ...
    return Dialog(
      ...
      child: SizedBox(
        ...
        child: ChangeNotifierProvider.value(
          value: controller,
          child: Consumer<CostRoomController>(
            child: const Center(
  ...
  FloatingActionButton get floatingActionButton => FloatingActionButton(
        ...
        onPressed: () => showDialog(
            context: context,
            builder: (context) => AddAccountingDialog(idRoom: widget.idRom!)),
