...
class ListMinibarHotelService extends StatefulWidget {
  const ListMinibarHotelService({Key? key}) : super(key: key);

  @override
  State<ListMinibarHotelService> createState() =>
      _ListMinibarHotelServiceState();
}

class _ListMinibarHotelServiceState extends State<ListMinibarHotelService> {
  ...
  Widget buildContent(bool isMobile) {
    ...
    if (MinibarManager().minibars.isEmpty) {
      return Center(
  ...
  List<Container> buildContentInPC() {
    return MinibarManager().minibars.map((minibar) {
      return Container(
        height: SizeManagement.cardHeight,
        margin: const EdgeInsets.symmetric(
        ...
        child: Row(
          children: [
            ...
            //id
            Expanded(child: NeutronTextContent(message: minibar.id!)),
            const SizedBox(width: SizeManagement.dropdownLeftPadding),
            //name
            Expanded(
              flex: 2,
              child: NeutronTextContent(
                  tooltip: minibar.name, message: minibar.name!),
            ),
            //price
            Expanded(
                child: NeutronTextContent(
              ...
              message: NumberUtil.numberFormat
                  .format(MinibarManager().getPriceOfItem(minibar.id!)),
            )),
              ...
              child: IconButton(
                ...
                onPressed: () async {
                  await showDialog(
                      context: context,
                      builder: (ctx) =>
                          MinibarHotelServiceDialog(item: minibar));
                },
              ),
            ),
            //delete-button
                ...
                onPressed: () async {
                  bool? confirm = await MaterialUtil.showConfirm(
                      ...
                      MessageUtil.getMessageByCode(
                          MessageCodeUtil.CONFIRM_DELETE_X, [minibar.name!]));
                  if (confirm == null || !confirm) {
                    return;
                  }
                  HotelItem itemDelete = HotelItem.copy(minibar)
                    ..type = ItemType.other;
      ...
      return Container(
        ...
        child: ExpansionTile(
          ...
          title: Row(
            children: [
              Expanded(
                child: NeutronTextContent(message: minibar.name!),
              ),
          ...
          children: [
            ...
            Row(
              children: [
                ...
                Expanded(
                  child: NeutronTextContent(message: minibar.id!),
                )
              ],
            ),
            const SizedBox(height: 15),
