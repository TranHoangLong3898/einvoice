...
import '../../../controller/booking/costcontroller.dart';
...
import '../management/accounting/accountingtypedialog.dart';
import '../management/accounting/addactualpaymentdialog.dart';

class CostBookingDialog extends StatefulWidget {
  final Booking booking;
  const CostBookingDialog({Key? key, required this.booking}) : super(key: key);

  @override
  State<CostBookingDialog> createState() => _CostBookingDialogState();
}

class _CostBookingDialogState extends State<CostBookingDialog> {
  late CostBookingController controller;
  double totalAmout = 0;

  @override
  void initState() {
    super.initState();
    controller = CostBookingController(widget.booking);
  }
    ...
    return Dialog(
      ...
      child: SizedBox(
        ...
        child: ChangeNotifierProvider.value(
          value: controller,
          child: Consumer<CostBookingController>(
            child: const Center(
  ...
  FloatingActionButton get floatingActionButton => FloatingActionButton(
        ...
        onPressed: () => showDialog(
            context: context,
            builder: (context) => AddAccountingDialog(booking: widget.booking)),
  ...
  ListView buildContentInMobile() {
    totalAmout = 0;
    return ListView(
      children: controller.accountings.map(
        (e) {
          totalAmout += e.amount!;
          return Container(
            decoration: BoxDecoration(
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    Expanded(
                        flex: 2, child: NeutronTextContent(message: e.status!))
                  ],
                ),
                const SizedBox(height: 8),
                Row(
